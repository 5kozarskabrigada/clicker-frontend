...
function updateUI() {
    ...
    coinsPerClickEl.textContent = userData.coins_per_click;
    coinsPerSecEl.textContent = userData.coins_per_sec;
...
async function init() {
    try {
        ...
        userData = await apiRequest('/user');
        updateUI();

        setInterval(async () => {
            if (userData?.coins_per_sec > 0) {
                const coins = parseInt(coinsEl.textContent.replace(/,/g, '')) || 0;
                coinsEl.textContent = (coins + userData.coins_per_sec).toLocaleString();
            }
        }, 1000);
        ...
        document.body.innerHTML = `
            <div class="error-container">
                ...
                <p>${error.message}</p>
                <p>Please try restarting the app via Telegram.</p>
            </div>
        `;
    }
}
